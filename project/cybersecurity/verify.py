...
for p in sorted(out.glob('*.mp4')):
    ...
    first=out/'qa'/(p.stem+'-first.png')
    subprocess.run(['ffmpeg','-v','error','-y','-i',str(p),'-frames:v','1',str(first)],check=True)
    a0=np.asarray(Image.open(first).convert('RGB'))
    changed=int((np.abs(a.astype(np.int16)-a0.astype(np.int16)).max(axis=2)>12).sum())
    assert changed>100,(p,'No visible internal animation',changed)
    ...
    results.append({'file':p.name,'width':v['width'],'height':v['height'],'fps':v['r_frame_rate'],'frames':int(v['nb_read_frames']),'seconds':float(meta['format']['duration']),'codec':v['codec_name'],'bitrate':int(v['bit_rate']),'bytes':p.stat().st_size,'green_fraction':round(float(green.mean()),4),'changed_pixels_0_to_2_5_seconds':changed,'decode':'PASS','sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
