...
const index = path.resolve('delivery/chat-to-motion-flappy/index.html');
...
for (const t of checkpoints){
  ...
  const hits=await page.evaluate(({textSelectors,objectSelectors})=>{
    const opacity = el => {
      let o=1,n=el;
      while(n && n.nodeType===1){const s=getComputedStyle(n);o*=Number(s.opacity||1);if(s.display==='none'||s.visibility==='hidden') return 0;n=n.parentElement;}
      return o;
    };
    ...
    const out=[];
    for(const ts of textSelectors){for(const te of document.querySelectorAll(ts)){if(opacity(te)<.12)continue;const tr=rect(te);if(tr.w<2||tr.h<2)continue;for(const os of objectSelectors){for(const oe of document.querySelectorAll(os)){if(opacity(oe)<.12)continue;const or=rect(oe);if(or.w<2||or.h<2)continue;const area=intersect(tr,or);if(area>24)out.push({text:ts,object:os,area,te:tr,oe:or});}}}}
    return out;
